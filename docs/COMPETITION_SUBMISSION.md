...
| 训练数据是否科学 | 训练包 `v5_16_synth_capped_rerender_official` 共 `21504` 行；训练材料、训练侧合成样本和同标签重渲染分开记录；缺图、空标签、替换符、公式化片段均为 `0`。 | [TRAINING_DATA_CONSTRUCTION_REPORT.md](TRAINING_DATA_CONSTRUCTION_REPORT.md)，[configs/train_data_manifest_v5_16.json](../configs/train_data_manifest_v5_16.json) |
...
| 官方维度 | 本项目最强证据 | 当前边界 |
|---|---|---|
| 评估集质量 | 主评估 `603` 条真实样本，含单行图 `515` 条、区域图 `84` 条、整页图 `4` 条；场景覆盖新印刷 `309`、旧印刷 `235`、规范手写 `53`、实拍/屏幕 `6`；难度覆盖简单 `172`、中等 `324`、困难 `107`。 | 主评分集未到 1000+ 样本；复杂整页压力测试不混入 603 主结果夸大精度。 |
| 场景稀缺性 | 规范彝文公开文字识别数据少；项目覆盖旧书、教材、工具书、彝汉混排、注音、手写和实拍资料。 | 应用目标限定在检索、校对、注音、语料建设和教学工作流。 |
| 任务复杂度 | 整页、PDF 和照片先经 PP-DocLayout 切成文本区域，再识别、恢复阅读顺序并合并页面文本；仓库提供整页演示和线上入口。 | 不声称已经稳定解决所有复杂整页；复杂整页仍建议人工复核。 |
| 训练数据构建科学性 | `21504` 行训练包，训练材料、训练侧合成样本、同标签重渲染分开记录；拉丁注音重渲染 `0`，脚注重渲染 `60`，多行区域重渲染 `649`；缺图、空标签、替换符、公式化片段均为 `0`。 | GitHub 保留配置、训练包清单、脚本和统计；权重与评估集分别放在 Hugging Face。 |
| 微调策略与创新 | 三阶段 LoRA 微调；训练侧用同标签重渲染补字体和旧印刷退化覆盖，并限制拉丁注音、脚注、多行区域等容易造成异常输出的样本比例。 | 不改 PaddleOCR-VL 结构，重点是训练数据构建、隔离检查和输出风险检查。 |
...
单行图输入最稳定。区域图和整页图用于暴露漏行、阅读顺序和换行边界问题。规范手写单独报告，不和印刷体混成一个结论。
...
在评估集《雪族子史篇》65 页整页样本上，`PP-DocLayout` 页面切割后识别的平均归一化编辑距离为 `0.0654`，直接整页识别为 `0.5540`。该对比说明页面切割能降低复杂整页中的阅读顺序错位、彝汉混排拆散和非文字花纹误识别。
...
最终训练包使用同标签重渲染：文本标签不变，只改变字体、退化和页面视觉状态。拉丁注音、脚注、多行区域样本设置上限，避免某类格式被模型学成默认输出习惯。
...
| 最终提交分支 | 同标签重渲染 `9069` 行，拉丁注音不放大，脚注/多行区域设上限 | 整体 `0.0342`，彝文 `0.0372`，汉字 `0.0225`，公式化片段 `2`，异常长输出 `0` | 最终提交模型 |
...
| 手写识别是否已经稳定 | 否。规范手写已有泛化信号，但明显弱于印刷体，单独报告 |
