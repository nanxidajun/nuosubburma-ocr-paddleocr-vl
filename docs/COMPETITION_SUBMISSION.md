...
| 模型结果是否已经跑完 | 是。提交模型在 `603` 条主评估样本上的平均归一化编辑距离（Avg NED，越低越好）为 `0.036068`；只看彝文字符为 `0.038309`，只看汉字为 `0.022447`。 | [evaluation/summary.md](../evaluation/summary.md)，[evaluation/raw/submission_model_result.jsonl](../evaluation/raw/submission_model_result.jsonl) |
| 是否只是单行识别 | 否。仓库能从整页、PDF、照片开始，使用 PP-DocLayout 做页面切割，记录阅读顺序，再识别切出的文本区域，最后合并页面文本并按需补注音；复杂整页仍建议人工复核。 | [页面切割流程](../page_processing/README.md)，[演示](../demo/README.md)，[后处理工具](../postprocess/README.md) |
| 训练数据是否科学 | 是。训练包 `v5_16_synth_capped_rerender_official` 共 `21504` 行；真实材料定任务边界，合成样本补未见字和低频字，标签不变的图像外观增强覆盖字体、清晰度和旧印刷状态；缺图、空标签和异常输出标签均为 `0`。 | [TRAINING_DATA_CONSTRUCTION_REPORT.md](TRAINING_DATA_CONSTRUCTION_REPORT.md)，[configs/train_data_manifest_v5_16.json](../configs/train_data_manifest_v5_16.json) |
...
| 官方维度 | 本项目最强证据 | 当前边界 |
|---|---|---|
| 评估集质量 | 主评估 `603` 条真实样本，含单行图 `515` 条、区域图 `84` 条、整页图 `4` 条；场景覆盖新印刷 `309`、旧印刷 `235`、手写拍照 `53`、实拍/屏幕 `6`；难度覆盖简单 `172`、中等 `324`、困难 `107`。 | 主评分集未到 1000+ 样本；复杂整页压力测试不混入 603 主结果夸大精度。 |
| 场景稀缺性 | 规范彝文公开文字识别数据少；项目覆盖旧书、教材、工具书、彝汉混排、注音、手写和实拍资料。 | 应用目标限定在检索、校对、注音和语料整理。 |
| 任务复杂度 | 整页、PDF 和照片先经 PP-DocLayout 切成文本区域，再识别、恢复阅读顺序并合并页面文本；仓库提供整页演示和线上入口。 | 不声称已经稳定解决所有复杂整页；复杂整页仍建议人工复核。 |
| 训练数据构建科学性 | `21504` 行训练包；真实材料定边界，合成样本补字符长尾，标签不变的图像外观增强覆盖字体、清晰度和旧印刷状态；拉丁注音不放大，脚注和多行区域设上限；缺图、空标签和异常输出标签均为 `0`。 | GitHub 保留配置、训练包清单、脚本和统计；权重与评估集分别放在 Hugging Face。 |
| 微调策略与创新 | 三阶段 LoRA 微调；文字标签不变，只扩展训练图像的字体、清晰度和旧印刷外观，并给拉丁注音、脚注、多行区域等高风险格式设比例上限。 | 不改 PaddleOCR-VL 结构，重点是训练数据构建、隔离检查和输出风险检查。 |
...
当前提交模型已在 `603` 条主评估样本上重新评估；原始模型结果保留脚本入口，可后续补齐。
...
| 维度 | 分布 |
|---|---|
| 输入粒度 | 单行图 `515`，区域图 `84`，整页图 `4` |
| 真实场景 | 新印刷扫描 `309`，旧书扫描 `235`，手写拍照 `53`，实拍/屏幕 `6` |
...
复杂整页切割对比与主评估集分开维护。主评估集只统计文字识别指标；页面切割演示只证明整页流程能跑通。
...
| 页面切割 | [页面切割流程](../page_processing/README.md) | 使用 PP-DocLayout 生成文本区域，降低复杂整页直接识别的漏行、错行和边界风险 |
...
| 注音 | [add_nuosu_pronunciation.py](../postprocess/add_nuosu_pronunciation.py) | 在页面文本完成后按需添加读音字段，供检字、校对和语料整理使用 |
| 整页演示 | [run_page_workflow.py](../demo/run_page_workflow.py) | 证明普通页面样例能按页面切割、识别、页面文本合并、异常检查和可选注音顺序跑通 |

在评估集《雪族子史篇》65 页整页样本上，`PP-DocLayout` 页面切割后识别的平均归一化编辑距离为 `0.0654`，直接整页识别为 `0.5540`。该对比说明页面切割能降低复杂整页中的阅读顺序错位、彝汉混排拆散和非文字图案误识别。

当前边界：复杂整页、手写段落、多栏、脚注/注音密集区域仍建议人工复核。模型可以输入整页图，但稳定交付优先使用可检查的文本区域。

## 6. 训练数据构建

训练集遵循“真实材料定边界、合成样本补长尾、同一文本换外观补视觉变化”的原则。
...
最终训练包使用标签不变的图像外观增强：文本标签不变，只改变字体、清晰度和旧印刷状态。拉丁注音、脚注、多行区域样本设置上限，避免某类格式被模型学成默认输出习惯。
...
三阶段调优策略如下：第一阶段验证 PaddleOCR-VL + LoRA 能否学习规范彝文；第二阶段用合成样本补低频字符和混排覆盖，并在文字标签不变的前提下扩展字体、清晰度和旧印刷外观；第三阶段只用固定开发诊断集比较分支输出，不把开发诊断集或最终评估集写回训练包。
...
| 最终提交分支 | 标签不变的图像外观增强 `9069` 行，拉丁注音不放大，脚注/多行区域设上限 | 整体 `0.0342`，彝文 `0.0372`，汉字 `0.0225`，公式化片段 `2`，异常长输出 `0` | 最终提交模型 |
...
可核验内容有四项：`603` 条真实主评估集、`21504` 行训练包、多组训练分支实验，以及整页/PDF/照片到文本区域识别、页面文本合并、异常检查和按需注音的流程。
