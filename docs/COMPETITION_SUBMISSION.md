...
本文档按 `2026-06-29` 最终评估集组织。评估集包含 `758` 条真实样本；历史 `603` 条 OCR 主指标结果作为已完成 LoRA 指标保留，`758` 条真实样本作为最终评估口径。

| 口径 | 状态 | 用途 |
|---|---|---|
| 最终评估集 | `758` 条真实样本，`line 470` / `region 119` / `page 169`，空 GT `0`，缺图 `0`，重复 ID `0`，合成标记 `0` | 提交主口径 |
| 原始模型 baseline | `PaddleOCR-VL-1.6` 用作未微调对照 | 与 LoRA 模型使用同一评估集和脚本 |
...
| 模型结果是否已经跑完 | 历史 `603` 条 LoRA 指标已完成：Avg NED `0.036068`，Yi-only Avg NED `0.038309`，Han-only Avg NED `0.022447`。最终 `758` 条评估集用于统一提交口径。 | [evaluation/summary.md](../evaluation/summary.md)，[evaluation/raw/submission_model_result.jsonl](../evaluation/raw/submission_model_result.jsonl) |
...
| 官方维度 | 本项目最强证据 | 当前边界 |
|---|---|---|
| 评估集质量 | 最终评估集 `758` 条真实样本；`line 470` / `region 119` / `page 169`；旧印刷 `507`、新印刷 `100`、手写拍照 `53`、真实照片 `11`、屏幕拍照 `87`；`easy 83` / `medium 467` / `hard 208`；空 GT、缺图、重复 ID、合成标记均为 `0`。 | `758` 条作为提交主口径；历史 `603` 条结果作为已完成模型指标保留。 |
| 场景稀缺性 | 规范彝文公开文字识别数据少；项目覆盖旧书、教材、工具书、彝汉混排、拉丁注音、手写、实拍照片和屏幕拍照资料。 | 应用目标限定在检索、校对、注音和语料整理。 |
| 任务复杂度 | 整页、PDF 和照片先经 Paddle DocLayout 切成文本区域，再识别、恢复阅读顺序、合并页面文本，并生成标题、正文、页码、块位置和彝汉对照行；《雪族子史篇》65 页切割后 OCR Avg NED `0.0504`，直接整页 OCR `0.5448`。 | 不声称已经稳定解决所有复杂整页；复杂整页仍建议人工复核。 |
...
当前提交模型已在历史 `603` 条 OCR 主指标样本上完成评估。最终 `758` 条真实样本作为原始模型和 LoRA 模型的统一评估口径。
...
最终评估集为 `758` 条真实来源样本，全部计入评估集口径，不拆成展示样本和评分样本两套。历史 `603` 条 OCR 主指标结果作为已完成模型结果保留。
