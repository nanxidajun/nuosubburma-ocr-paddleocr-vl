...
本文档按 `2026-06-29` 最终评估集组织。评估集包含 `758` 条真实样本；未微调基座结果已完成，LoRA 微调后结果未出，二者使用同一评估集和同一评估脚本。
...
| LoRA 微调后 | 结果未出 | 完成后按最终 `758` 条同口径回填 |
...
| 模型结果是否已经跑完 | 未微调基座已在最终 `758` 条评估集完成：Avg NED `0.726733`，Yi-only Avg NED `1.000000`，Han-only Avg NED `0.209245`。LoRA 微调后结果未出。 | [evaluation/summary.md](../evaluation/summary.md) |
...
最终 `758` 条真实样本作为未微调基座和 LoRA 微调模型的统一评估口径。未微调基座已完成；LoRA 微调后结果未出。
