...
| 训练数据是否科学 | 训练包 `v5_16_synth_capped_rerender_official` 共 `21504` 行；训练材料、训练侧合成样本和同标签重渲染分开记录；缺图、空标签、替换符、LaTeX-like 标签均为 `0`。 | [TRAINING_DATA_CONSTRUCTION_REPORT.md](TRAINING_DATA_CONSTRUCTION_REPORT.md)，[configs/train_data_manifest_v5_16.json](../configs/train_data_manifest_v5_16.json) |
| 模型选择是否有实验依据 | 有。v5.8、v5.15、v5.16、v5.17 等分支使用同一组开发诊断集比较；v5.17 对照显示，继续追加长尾格式样本会带来 LaTeX 和 long prediction 风险回升。 | [MODEL_AND_TRAINING.md](MODEL_AND_TRAINING.md)，[evaluation](../evaluation/) |
...
| 训练数据构建科学性 | `21504` 行训练包，训练材料、训练侧合成样本、同标签重渲染分开记录；Latin rerender `0`，footnote rerender `60`，region-like rerender `649`；缺图、空标签、替换符、LaTeX-like 标签均为 `0`。 | GitHub 保留配置、manifest、脚本和统计；权重与评估集分别放在 Hugging Face。 |
...
训练集遵循“真实训练材料打底、训练侧合成样本补覆盖、同标签重渲染补视觉变化”的原则。
...
三阶段调优策略如下：第一阶段验证 PaddleOCR-VL + LoRA 能否学习规范彝文；第二阶段用训练侧合成样本和同标签重渲染补低频字符、字体变化、旧印刷退化和彝汉混排；第三阶段只用固定开发诊断集比较分支输出，不把开发诊断集或最终评估集写回训练包。
