...
| 任务复杂度 | 整页、PDF 和照片先经 Paddle DocLayout 切成文本区域，再识别、恢复阅读顺序、合并页面文本，并导出标题、正文、页码、块位置和彝汉对照行。 | 不声称已经稳定解决所有复杂整页；复杂整页仍建议人工复核。 |
...
复杂整页切割对比与主评估集分开维护。主评估集只统计文字识别指标；雪族 65 页用于证明整页切割、OCR、拼合、结构化输出和审计链路能跑通。
...
这组整页压力测试已经端到端跑通：`529` 个 DocLayout 版面块，`2501` 个 OCR 单元，`2501` 条 OCR 结果全部正常，拼合得到 `65` 页页面文本；替换符、空页和重复页均为 `0`。结构化输出得到 `65` 个页面结构，包含 `1123` 行彝文原文和 `1060` 组彝汉对照行。
...
可核验内容有四项：`603` 条真实主评估集、`21504` 行训练包、多组训练分支实验，以及整页/PDF/照片到文本区域识别、页面文本合并、结构化输出、异常检查和按需注音的流程。
