...
模型用于把图片中的规范彝文、彝汉混排、数字、标点和可见 Latin 注音转写为 Unicode 文本。

典型输入包括书籍扫描页、行图、文本区域、屏幕拍照、实拍标牌和规范手写样本。
...
推荐从 GitHub 仓库运行本地 demo。模型推理建议使用 CUDA GPU 环境。
...
对复杂书页，先做页面切割再识别，通常比整页直接 OCR 更稳定。
...
| 合成样本补覆盖 | 用训练侧合成样本和同标签重渲染补低频字符、字体变化和旧印刷视觉变化 |
| 隔离检查 | 训练包和评估集分开维护；重合检查保留在 GitHub 配置和训练数据报告中 |
| 输出风险控制 | 训练和开发诊断时观察 replacement、LaTeX-like、extra Latin、long prediction 等错误 |
...
- 本版本尚未做端侧或移动端优化。
- Space Demo 需要 GPU 才能稳定运行模型；没有 GPU 时，以本地 demo 与脚本为准。
