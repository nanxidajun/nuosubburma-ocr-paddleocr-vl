...
最终评估集为 `758` 条真实来源样本，含 `line 470` / `region 119` / `page 169`，空 GT、缺图、重复 ID 和合成样本标记均为 `0`。历史 `603` 条 OCR 主指标作为已完成模型指标保留。

模型卡 metadata 中的 NED 指标来自历史 `603` 条 OCR 主指标，保留为已经完成的可核验结果，不代表最新 `758` 条最终分数。
...
在总评估材料中的《雪族子史篇》65 页整页子集上，两种路径的对比如下。历史 OCR 指标、最新 `758` 条正式评估和雪族 65 页整页切割对照分表报告，口径不同，不合并成一个平均分。
...
对复杂书页，先做页面切割再识别，通常比整页直接识别更稳定。该子集包含 `65` 页、`2501` 个 OCR 单元和 `65` 页拼合文本；替换符、空页和重复页均为 `0`。

## 评估结果

当前公开 NED 结果来自历史 `603` 条 OCR 主指标；最新 `758` 条整合评估集作为最终评估口径。合成样本不进入评估结果。归一化编辑距离（NED）越低越好。
...
本文仅报告已完成并可核验的历史 `603` 条 OCR 主指标，不编造缺失结果。
