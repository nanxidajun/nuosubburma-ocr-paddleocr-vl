...
# NuosuBburma OCR：规范彝文 OCR 模型

`NuosuBburma OCR` 是面向规范彝文（ꆈꌠꁱꂷ / Nuosu Bburma）的 OCR 模型，基于 `PaddleOCR-VL-1.6 (0.9B)` / `PaddlePaddle/PaddleOCR-VL` 进行 LoRA 微调。

模型用于把真实资料中的规范彝文、彝汉混排、数字、标点和可见 Latin 注音转写为 Unicode 文本。典型输入包括旧书扫描、教材页面、工具书页面、行图、文本区域、屏幕拍照、手机页面照片、实拍标牌和规范手写样本。
...
| Hugging Face Space Demo | <https://huggingface.co/spaces/nanxidajun/NuosuBburma-OCR-Demo> |

## 模型信息

| 项目 | 内容 |
|---|---|
| 公开名称 | `NuosuBburma OCR` / `规范彝文 OCR` |
...
| 稳定输入 | line / region 图像 |
| 扩展输入 | page、PDF 渲染页、手机拍照页、屏幕拍照、实拍标牌 |
| 推荐整页流程 | 先用 `PP-DocLayout` 生成 OCR 单元，再识别并合并页面文本 |

## 快速使用

推荐从 GitHub 仓库运行本地 demo。完整模型推理建议使用 CUDA GPU 环境。
...
输入为包含规范彝文或彝汉混排文本的图片，输出为图片中可见文本。注音是后处理步骤，不会改写 OCR 正文。
...
这个结果说明：对复杂书页，先处理版式再识别文字，通常比整页直接送入 OCR 模型更稳定。

## 评估结果

当前公开结果使用 `NuosuBburma OCR Evaluation Set` 的主评估样本，合成样本不进入主评估。NED 越低越好。
...
| replacement / LaTeX-like / extra Latin / long prediction | `0 / 2 / 0 / 0` |

指标解释：

| 指标 | 中文说明 |
|---|---|
| Avg NED | 平均归一化编辑距离，越低越接近人工 GT |
...
| replacement / LaTeX-like / extra Latin / long prediction | 输出风险审计项，用于检查替换符、公式化片段、多余 Latin 和异常长输出 |

Base 模型、第一阶段、第二阶段和最终模型的同一评估设置对比会在最终评估集冻结并完成评估后补充。
...
| 原则 | 做法 |
|---|---|
| 真实样本打底 | 使用旧书、教材、工具书、混排、手写和页面照片等真实来源建立任务边界 |
| 合成样本补覆盖 | 用受控重渲染补低频字、形近字、字体和旧印刷视觉变化 |
| 评估集隔离 | 最终评估集不作为训练目标，训练/评估重合检查保留在 GitHub 配置和报告中 |
| 输出风险控制 | 重点检查 replacement、LaTeX-like、extra Latin、long prediction 等错误 |
| 分支按结果晋级 | v5.16 综合 NED 和风险指标最好；v5.17 虽更新但未作为最终模型 |
...
- 清晰 line / region 输入是当前最稳定的使用方式。
- 直接 page OCR 可作为诊断入口，但复杂整页建议使用 `PP-DocLayout` 页面切割后识别。
- 模型输出是 OCR 结果，不是自动校对结果；重要文本仍建议人工复核。
- 注音是 OCR 后处理，不参与模型评分，也不会改写 OCR 正文。
- 本版本尚未做端侧或移动端优化。
- Space Demo 需要 GPU 才能稳定运行完整模型；没有 GPU 时，以本地 demo 与脚本复现为准。

## 许可与合规

本模型基于 `PaddlePaddle/PaddleOCR-VL` 微调发布。使用者需要同时遵守基座模型、PaddleOCR/PaddlePaddle 相关依赖、Hugging Face 托管条款以及数据来源的版权边界。

本模型主要面向比赛评审、研究和规范彝文资料数字化实验。商业使用前请自行确认基座模型、数据来源和部署环境的授权条件。
