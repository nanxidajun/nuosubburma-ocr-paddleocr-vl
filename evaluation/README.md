...
最终提交评估表使用同一 `NuosuBburma OCR Evaluation Set`、同一脚本和同一指标。当前提交模型已经完成 `603` 条主评估样本统计；PaddleOCR-VL Base、第一阶段、第二阶段和最终模型结果将在最终评估集冻结后按同一脚本补充，可后续补齐。
...
本目录不保留训练日志、评估运行日志、人工审查中间表、复杂整页压力测试材料和全量错误工作表，避免公开仓库变成实验过程垃圾堆。复现脚本见 [`../scripts/`](../scripts/)。
