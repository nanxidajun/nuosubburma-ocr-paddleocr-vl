...
本目录中的图表和逐条输出来自历史 `603` 条 OCR 主指标统计。最新整合评估集已经扩展到 `758` 条真实来源样本；最终 `758` 条同口径结果等待正式 rerun 后补充。

原始 `PaddleOCR-VL-1.6` baseline 已完成 `758` 条 aligned raw prediction，错误行 `0`。原始模型和 LoRA 模型的最新 NED、exact 和输出风险指标将在同一脚本下正式计算。

复杂整页切割/版面对比材料不放在本 OCR 模型包的 `evaluation/` 中；本目录只保留历史 OCR 指标相关的模型输出、图表和分组统计。
...
结论：新旧印刷体表现稳定，手写拍照和照片/真实场景作为独立分组观察，不与清晰印刷行图混算。

### 不同来源

![Avg NED by Source](charts/ned_by_source.svg)

结论：旧印刷资料选译、语法书和《勒俄特依》译注等来源表现较强；手写拍照资料单独报告，避免掩盖不同来源的识别差异。
...
本目录不保留训练日志、评估运行日志、人工审查中间表、整页切割对照材料和全量错误工作表。复现脚本见 [`../scripts/`](../scripts/)。
