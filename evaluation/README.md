...
本目录中的图表和逐条输出来自历史 `603` 条 OCR 主指标统计。最终评估集为 `758` 条真实来源样本；两者分开报告，避免把历史指标和最终数据集规模混写。
...
| LaTeX-like outputs | `2` | 有 `2` 条脚注/符号公式化输出需要单独核验 |
...
结论：此前最危险的替换符崩溃和超长输出没有出现；LaTeX-like 残留为 `2` 条，作为输出风险单独报告。
...
本目录不保留训练日志、评估运行日志、人工审查表、整页切割对照材料和全量错误工作表。复现脚本见 [`../scripts/`](../scripts/)。
