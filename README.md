...
书籍扫描是当前最可靠的适用范围；手写拍照是最明显的短板。少数带装饰图案、部首检字表或屏幕摩尔纹的页面会触发异常生成、抬高总体误差。完整逐类分析与异常页清单见 [评估结果分析](docs/EVALUATION_RESULT_ANALYSIS.md)，评估集标注质量见 [评估质量报告](docs/EVALUATION_QUALITY_REPORT.md)。
