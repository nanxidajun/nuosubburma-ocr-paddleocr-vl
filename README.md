...
最终评估统一使用 `758` 条真实来源样本，对比 `PaddleOCR-VL-1.6` 未微调基座和 LoRA 微调后的提交模型。未微调基座结果已经完成：Avg NED 为 `0.7267`，Yi-only Avg NED 为 `1.0000`，并出现 `16` 条替换符、`105` 条公式化片段、`321` 条多余 Latin 和 `34` 条异常长输出。这说明原始通用 OCR 模型几乎不能直接承担规范彝文识别任务。

LoRA 微调后结果未出，完成后会用同一评估集、同一图片、同一标注和同一评分脚本回填。首页只保留关键对比结论：本项目的核心不是单独报告一个模型分数，而是验证 LoRA 微调能否把通用 PaddleOCR-VL 从“不适配规范彝文”推进到可用于真实彝文资料整理的 OCR 模型。
