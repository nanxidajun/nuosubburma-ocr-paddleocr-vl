...
| 场景稀缺性 | 规范彝文公开 OCR 资源少；本项目覆盖旧书、教材、工具书、彝汉混排、注音、手写和实拍资料 | [项目背景](docs/PROJECT_BACKGROUND.md) |
| 任务复杂度 | 整页、PDF 和照片先经 PP-DocLayout 切割为 OCR 单元，再识别、恢复阅读顺序并合并页面文本 | [页面切割](page_processing/README.md)，[demo](demo/README.md) |
| 训练数据科学性 | 训练包 `21504` 行；训练材料、训练侧合成样本和同标签重渲染分开记录，并检查缺图、空标签、替换符和 LaTeX-like 标签 | [训练数据构建](docs/TRAINING_DATA_CONSTRUCTION_REPORT.md)，[manifest](configs/train_data_manifest_v5_16.json) |
| 微调策略与创新 | 三阶段 LoRA 微调；训练侧用同标签重渲染补字体和旧印刷退化覆盖，并对 Latin、脚注、region-like 多行设置上限 | [模型与训练](docs/MODEL_AND_TRAINING.md)，[训练数据构建](docs/TRAINING_DATA_CONSTRUCTION_REPORT.md) |
...
当前提交模型在 `603` 条主评估样本上取得 Avg NED `0.036068`。Yi-only Avg NED 为 `0.038309`，Han-only Avg NED 为 `0.022447`。

replacement / LaTeX / extra Latin / long_pred 为 `0 / 2 / 0 / 0`。

清晰 line/region 是当前可靠入口，复杂整页和规范手写单独报告。
...
| 页面切割与 OCR 流程 | 从整页、PDF、照片开始，切出 OCR 单元，按阅读顺序合并页面文本，并按需补充注音输出 | [页面切割流程](page_processing/README.md)，[demo](demo/README.md)，[后处理工具](postprocess/README.md) |
| 训练数据构建 | 训练材料、训练侧合成样本、同标签重渲染和训练侧约束样本分开记录，配合训练包质检、隔离检查和 manifest | [训练数据构建](docs/TRAINING_DATA_CONSTRUCTION_REPORT.md)，[configs](configs/) |
...
| 阅读顺序记录 | `page_id`、`crop_id`、`reading_order`、`bbox` | 支持把 OCR 单元识别结果合并为页面文本 |
| 彝文/彝汉混排 OCR | `<image>OCR:` | 输出规范彝文、汉字、数字、标点和可见 Latin 注音 |
| 页面文本合并 | [assemble_pages.py](page_processing/assemble_pages.py) | 按几何位置、块角色和 reading order 做 visual-line 拼合，生成可校对页面文本 |
...
| 整页 demo | [run_page_workflow.py](demo/run_page_workflow.py) | 跑通页面切割、OCR 单元识别、页面文本合并、异常审计和可选注音 |
...
这个实验说明：复杂整页先处理版式、再识别文字，通常比整页直接进入 OCR 更稳定。详细说明见 [页面切割流程](docs/PAGE_PROCESSING.md)。

## 评估与训练

最终结果按 `NuosuBburma OCR Evaluation Set` 统计。PaddleOCR-VL Base 和当前提交模型使用同一评估集、脚本和指标。
...
训练侧用真实训练材料打底，用训练侧合成样本和同标签重渲染补覆盖，并限制 Latin、脚注、region-like 多行等高风险通道。
