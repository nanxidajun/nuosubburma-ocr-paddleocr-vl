...
输入一张图片，**端到端**输出其中规范彝文（ꆈꌠꁱꂷ / Nuosu Bburma）、汉字、拉丁字母、数字和标点的 Unicode 文本——单模型一次推理，不做文字分类、双模型路由或检测识别切割。输出的可搜索、可校对文本可直接进入语料库，支撑规范彝文的检索、注音校对与低资源 NLP。

[模型](https://huggingface.co/nanxidajun/NuosuBburma-OCR) · [评估集](https://huggingface.co/datasets/nanxidajun/NuosuBburma-OCR-Evaluation-Set) · [评估集可视化](https://huggingface.co/spaces/nanxidajun/NuosuBburma-OCR-Evaluation-Set) · [在线演示](https://huggingface.co/spaces/nanxidajun/NuosuBburma-OCR-Demo) · [项目背景](docs/PROJECT_BACKGROUND.md) · [模型与训练](docs/MODEL_AND_TRAINING.md) · [训练数据构建](docs/TRAINING_DATA_CONSTRUCTION_REPORT.md)
...
![评估结果分类概览](docs/figures/evaluation-classification-overview.png)

分组 NED（去空白口径）：

| 采集场景 | 样本 | NED | | 难度 | 样本 | NED | | 文字构成 | 样本 | NED |
|---|--:|--:|---|---|--:|--:|---|---|--:|--:|
| 书籍扫描 | 787 | 0.0259 | | 低 | 271 | 0.0163 | | 彝汉混排 | 742 | 0.0249 |
| 屏幕拍照 | 164 | 0.0393 | | 中 | 501 | 0.0239 | | 纯彝文 | 204 | 0.0700 |
| 手写拍照 | 70 | 0.2000 | | 高 | 258 | 0.0957 | | 彝汉拉丁 | 84 | 0.0994 |
| 实景拍照 | 9 | 0.0247 | | | | | | | | |

书籍扫描是当前最可靠的适用范围；手写拍照是最明显的短板；彝汉拉丁混排误差最高，拉丁字母、数字、标点与彝汉相邻时的边界与顺序仍不稳定。少数带装饰图案、部首检字表或屏幕摩尔纹的页面会触发异常生成、抬高总体误差。完整逐类分析与异常页清单见 [评估结果分析](docs/EVALUATION_RESULT_ANALYSIS.md)，评估集标注质量见 [评估质量报告](docs/EVALUATION_QUALITY_REPORT.md)。
...
- 形近字专项微调降低了总体错误，但未解决全部形近字，个别高频混淆仍存在。
- 严重倾斜、反光、遮挡、弯曲、低清与复杂版式仍可能影响结果。
- 约 0.9B 参数的高精度参考模型，尚未针对移动端优化。

## 许可

模型基于 PaddleOCR-VL-1.6 微调发布，上游在固定版本 `66317acc4c9fc17bd154591ce650735cd2855f3e` 处核验为 Apache-2.0。评估集许可见 [Hugging Face 评估集](https://huggingface.co/datasets/nanxidajun/NuosuBburma-OCR-Evaluation-Set)。使用时需同时遵守上游项目与数据来源的相关许可。
