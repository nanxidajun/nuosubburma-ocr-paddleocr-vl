...
| 合成数据容易带来漂移 | 用人工复核真实样本检查错误模式，再结合输出风险指标选择模型 |
...
训练从一本旧书《勒俄特依》的真实裁切行开始。第一阶段先确认 PaddleOCR-VL LoRA 能学到规范彝文字形，并产出可用于人工核对的预标注能力；第二阶段加入合成覆盖和监控集，补低频字、形近字和旧印刷变化；第三阶段再用人工复核真实样本检查错误模式，避免模型只适应合成样本或单一本书。

第三阶段的真实数据更像一张诊断表。它不是拿来让模型背题，也不是追某几个字的得失，而是观察模型在换书、混排、脚注、数字、手写、region/page 这些真实场景里会不会反复犯同类错误：输出漂移、GT 外 Latin 尾巴、符号 LaTeX 化、漏行、长输出、页面边界错。模型分支最后看的是这些错误模式和 NED 的整体变化。
...
- [评估集质检报告](docs/EVALUATION_QUALITY_REPORT.md)
- [训练数据构建报告](docs/TRAINING_DATA_CONSTRUCTION_REPORT.md)
