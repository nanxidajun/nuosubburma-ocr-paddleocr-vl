...
按难度看，模型在简单和复杂样本上已经比较稳定，困难样本仍主要受整页、长文本、复杂版式和屏幕拍摄影响。

| 难度 | rows | Avg NED | Exact | 说明 |
|---|---:|---:|---:|---|
...
按输入粒度看，line `470` 条 Avg NED `0.025444`，region `119` 条 Avg NED `0.082315`，page `169` 条 Avg NED `0.186774`。按真实场景看，旧印刷/扫描资料 `507` 条 Avg NED `0.036873`、新印刷/PDF `100` 条 Avg NED `0.053856`、真实场景照片 `11` 条 Avg NED `0.011364`，这些类型已经可以作为资料整理和人工校对的基础结果；手写拍照 `53` 条 Avg NED `0.124483` 仍需要复核；屏幕拍照/页面上传图表现最差，`87` 条 Avg NED `0.258809`、Exact `3/87 (3.4%)`，是后续最需要针对性优化的场景。
