...
| 训练数据科学性 | 训练包 `21504` 行；训练材料、合成样本和同标签重渲染分开记录，并检查缺图、空标签、替换符和 LaTeX-like 标签 | [训练数据构建](docs/TRAINING_DATA_CONSTRUCTION_REPORT.md)，[manifest](configs/train_data_manifest_v5_16.json) |
...
| 训练数据构建 | 训练材料、受控合成和同标签重渲染分开记录，配合训练包质检、隔离检查和 manifest | [训练数据构建](docs/TRAINING_DATA_CONSTRUCTION_REPORT.md)，[configs](configs/) |
...
训练侧用真实训练材料打底，用受控合成和同标签重渲染补覆盖，并限制 Latin、脚注、region-like 多行等高风险通道。
