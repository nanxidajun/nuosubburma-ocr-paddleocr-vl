...
彝语使用者约 300 万；本项目面向记录彝语的规范彝文（ꆈꌠꁱꂷ）及其与汉字、拉丁字母、数字和标点的混排识别。教材、诗歌、民族文献和日常资料多以图片、扫描件和纸质书稿形式存在，难以检索、复制和整理，而通用 OCR 尚不能稳定识别。本项目采用**端到端单模型**：整幅图片直接输入、按视觉阅读顺序直接输出整页文字，不依赖版面切割流程，覆盖书籍扫描、屏幕拍照、手写和实景等真实场景。
...
以《雪族》（凉山彝文资料，64 页真实扫描）为例，在同一审定 GT、同一 NED 口径下对比未微调基座 PaddleOCR-VL-1.6 与最终微调模型：

| 模型 | 平均 NED | CER |
|---|---:|---:|
| 未微调基座 PaddleOCR-VL-1.6 | 0.5082 | 0.5989 |
| 最终微调模型 | **0.0259** | **0.0206** |
