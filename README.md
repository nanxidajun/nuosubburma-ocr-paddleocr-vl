...
|---|---:|---:|---|
| Avg NED | `0.726733` | `0.070123` | 整体错误量下降约 `90%`，微调是决定性收益。 |
| Exact | `0 / 758` | `447 / 758 (59.0%)` | 在短文本和清晰行图上已经有较高完全匹配率。 |
| Yi-only Avg NED | `1.000000` | `0.069797` | 基座几乎不能稳定输出规范彝文，LoRA 后彝文主干可用。 |
| Han-only Avg NED | `0.209245` | `0.055544` | 混排中的汉字也明显改善。 |
| replacement / LaTeX-like / extra Latin / long prediction | `16 / 105 / 321 / 34` | `0 / 6 / 1 / 1` | 异常输出基本被压住。 |

按拆分结果看，最稳定的是单行图（`470` 条，Avg NED `0.025444`）和旧印刷/扫描资料（`507` 条，Avg NED `0.036547`）。主要短板是整页图（`169` 条，Avg NED `0.185795`）、屏幕拍照/页面上传图（`87` 条，Avg NED `0.258809`）和手写拍照（`53` 条，Avg NED `0.124483`）。因此当前模型已经适合做真实资料整理和人工校对的第一稿；复杂页面和拍照类输入仍建议配合页面切割、结构化输出和人工复核。
