...
规范彝文（ꆈꌠꁱꂷ）使用者约 300 万，教材、诗歌、民族文献和日常资料多以图片、扫描件和纸质书稿形式存在，难以检索、复制和整理，而通用 OCR 尚不能稳定识别。本项目把它做成端到端识别任务：整页图片直接输入，按视觉阅读顺序输出彝文、汉字、拉丁字母、数字和标点的 Unicode 文本，覆盖书籍扫描、屏幕拍照、手写和实景等真实场景，不切行、不切块、不做多模型路由。
