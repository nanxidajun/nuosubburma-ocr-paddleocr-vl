...
低资源文字 OCR 的难点不只是“训练样本少”。更现实的问题是：真实资料难整理、标注成本高、字符覆盖不均、合成数据容易改变分布，模型还会在混排和符号附近出现输出漂移。这个项目把这些问题拆成一条可复查的资料整理链路：真实资料进入，OCR 输出 Unicode 文本，后续继续接校对、检字、注音和语料建设。
